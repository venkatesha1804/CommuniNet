...
export default function EditAccommodationPage() {
    ...
    if (authLoading || isPageLoading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#FAF9F6]">
            <div className="relative">
                <div className="h-24 w-24 border-4 border-slate-100 border-t-secondary rounded-full animate-spin"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                    <Building2 className="h-10 w-10 text-slate-900 animate-pulse" />
                </div>
            </div>
        </div>
    )
        ...
        <div className="min-h-screen flex flex-col bg-[#FAF9F6]">
            ...
            <main className="flex-1 py-12">
                <div className="container mx-auto px-4 max-w-3xl">
                    ...
                    <Link href={`/accommodations/${params.id}`} className="inline-flex items-center text-slate-400 hover:text-slate-900 transition-all mb-8 font-black uppercase tracking-[0.2em] text-[10px] group bg-white px-6 py-3 rounded-full shadow-lg border border-slate-50">
                        <ArrowLeft className="h-4 w-4 mr-2 group-hover:-translate-x-1 transition-transform" /> Back to Listing
                    </Link>
                    ...
                    <div className="mb-10 text-center">
                        ...
                        <p className="text-slate-500 font-bold text-lg">Update your hostel details to keep information accurate for tenants.</p>
                    </div>
                    ...
                    <div className="bg-white border border-slate-100 shadow-2xl shadow-slate-200/50 rounded-[3rem] overflow-hidden">
                        <div className="bg-slate-900 p-10 text-white relative overflow-hidden group">
                            ...
                            <h3 className="text-2xl font-black relative z-10 tracking-tight text-white uppercase">Property Control</h3>
                            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest relative z-10 mt-2">Managing: <span className="text-secondary">{formData.name}</span></p>
                        </div>
                                ...
                                <div className="mt-1 flex justify-center px-6 pt-16 pb-16 border-4 border-slate-100 border-dashed rounded-[3rem] bg-slate-50/50 hover:bg-white hover:border-secondary transition-all cursor-pointer group">
                                    <div className="space-y-6 text-center">
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-lg font-black text-slate-900">Update Photos</p>
                                            ...
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Choosing new photos will REPLACE all existing ones.</p>
                                        </div>
                                        {images.length > 0 && (
                                            <Button type="button" variant="ghost" size="sm" onClick={clearNewImages} className="text-xs font-black text-red-500 hover:text-red-600 hover:bg-red-50 rounded-full px-4">
                                                Clear Selected
                                            </Button>
                                        )}
                                    </div>
                                ...
                                {activeImagePreviews.length > 0 && (
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-8">
                                        {activeImagePreviews.map((url, i) => (
                                            <div key={url} className="relative h-32 rounded-[1.5rem] overflow-hidden border-2 border-white shadow-md group">
                                                <Image src={url} alt={`Preview ${i}`} fill className="object-cover transition-transform group-hover:scale-110" />
                                            </div>
                                        ))}
                                    </div>
                                )}
